angular.module('app')
 .controller('LocalisationsController',[ '$scope' , '$state','$stateParams','$location', 'leafletData','$uibModal', 'Restangular','$filter' , 'osmOverpassAPI','$rootScope','$http','svrUser','svrLocation','svrOverPass','$compile', 'notify','Upload',function($scope , $state , $stateParams,$location, leafletData ,$uibModal,Restangular,$filter,osmOverpassAPI,$rootScope,$http,svrUser,svrLocation,svrOverPass,$compile,notify,Upload){
...
// UTIL METHODS FOR CONTROLLER
        ...
        $scope.loadMap = function(obj , callback){
          ...
          leafletData.getMap(obj.scope).then(function(map){
           ...
           mapMenu.addItem({ text: 'Creer une addresse', callback: function(e){$scope.create_adresse(e,obj) } })


              // mapMenu.addItem({ text: 'Associer une addresse à moi', callback: function(e){$scope.associer_addresse(e) } })
            // mapMenu.addItem({ text: 'Definir un lieu de domicile', callback: function(e){$scope.definir_domicile(e) } })
            // mapMenu.addItem({ text: 'Definir un lieu de travail', callback: function(e){$scope.definir_travail(e) } })
    ...
    //pour que les gens puissent à partir de la carte principale creer une
              if(obj.scope){
                if(callback){
                  callback($scope.map)
                }
...
$scope.$on("ealo-adresse-created",function(e,v){

   $scope.show_adresse(v) ;

})

$scope.show_adresse = function(adresse){
  var place = L.polygon(adresse.location_points,{color:"#009900"}).bindPopup($scope.adresse_info(adresse))
      place.addTo($scope.map)

}

$scope.adresse_info = function(adresse){
 var addr ="<div>"+ adresse.location_address
       addr += "<hr/><div class='routing_tools'>"+
                "<a  href='#' uib-popover='favoris' popover-trigger='mouseenter' popover-placement='bottom' "+
                "ng-click='add_location_favourite("
                +adresse.building_id+
                ","+adresse.id+
                ")'><i class='fa fa-map-pin'></i></a>"+
                "<a href='#' uib-popover='partager' popover-trigger='mouseenter' popover-placement='bottom' ng-click=partage_location("
                +adresse.building_id+
                ")><i class='fa fa-share-alt'></i></a></div></div>";
       addr = $compile(addr);
       addr = addr($scope)[0]
       return addr;
}
...
$scope.modal_adresse = function(name , size){
  var size = size || "md"
  $scope.third_part_annonce_modal = $uibModal.open({
        templateUrl: 'app/templates/modals/'+name,
        size : 'lg',
        backdrop:'static',
        scope: $scope,
       windowClass:"partage",
...
$scope.create_main_adresse = function(e,m){
    $scope.selected_place = e;
      //$scope.map.setView([e.lat, e.lng], 16);
    if(m){
      m.removeFrom($scope.map)
    }

    $scope.modal_adresse("create_main_location_2.html")
}


  $scope.open_adresse = function(){
    $scope.modal_adresse("create_thirdpart_location_2.html");
  }

// // lorsque qu'on clique sure la carte on creer un marker qu'on peux ajuster
//
  $scope.create_adresse = function(e,obj) {
    $scope.map.setView([e.latlng.lat, e.latlng.lng], 18);
    $scope.cp = e.latlng;
    $scope.marker = L.marker([e.latlng.lat, e.latlng.lng],{draggable:true})
     var btn = '<div style="text-align:center">Ajuster votre localisation<br/><a style="color:#fff" ng-click="create_main_adresse(cp,marker)" class="btn btn-primary btn-xs"><i class="fa fa-check"></i> valider </a></div>'
...
$scope.link ="http://localhost:3000/#/localisations/share?wp="+ $scope.link_part
 ...
 control.on("routesfound",function(e){
  ...
  $scope.init_around = function(){
    Restangular.one("/annonces.json/").get()
    .then(function(elements){
      $scope.annonces = elements
...
$scope.upload = function (files,data) {
      ...
      return Upload.upload({
          url: '/annonces/',
          data : {files : files , info_data : JSON.stringify(data)}
      });
  }


   $scope.sent_depose_annonce = function(){
     console.log($scope.annonce)
    var response =  $scope.upload($scope.myCroppedImage, $scope.annonce)
      $scope.close_deposer_annonce()
      console.log(reponse)
    ...
    $scope.deposer_annonce = function(){
      $scope.depose_annonce_modal = $uibModal.open({
         templateUrl: 'app/templates/modals/depose_annonce_4.html',
         size : 'md',
         backdrop:'static',
         scope: $scope,
        windowClass:"partage",
...
// image slide
  ...
  $scope.imsl_init =function(name){
    ...
    // verifier si fermer un modal deja fermme entraine une erreur
    $scope.imsl_close_modal() ;
    $scope.imsl_actual_images =  $scope.imsl_get_images(name);
    $scope.imsl_open_modal();
  }
